...
uint8_t fpm_begin(FPM * fpm) 
{
    
     delay_ms(750);  // 500 ms at least according to datasheet
    
    fpm->buffer[0] = FPM_VERIFYPASSWORD;
    fpm->buffer[1] = (fpm->password >> 24) & 0xff;
    fpm->buffer[2] = (fpm->password >> 16) & 0xff;
    fpm->buffer[3] = (fpm->password >> 8) & 0xff;
    fpm->buffer[4] = fpm->password & 0xff;
